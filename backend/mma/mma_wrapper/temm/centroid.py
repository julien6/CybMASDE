...
def compute_action_centroids_per_cluster(clusters: Dict[int, List[Any]], mode: str = "mean") -> Dict[int, Any]:
    ...
    action_encoded_clusters = {cluster_label: [np.concatenate([np.eye(num_actions)[action] for action in action_trajectory])
                                               for action_trajectory in action_trajectories] for cluster_label, action_trajectories in clusters.items()}
...
def compute_full_centroids_per_cluster(clusters: Dict[int, List[Any]], mode: str = "mean") -> Dict[int, Any]:
    ...
    action_encoded_full_clusters = {cluster_label: [np.concatenate([np.concatenate((observation, np.eye(num_actions)[
                                                                   action])) for observation, action in action_trajectory]) for action_trajectory in action_trajectories] for cluster_label, action_trajectories in clusters.items()}
