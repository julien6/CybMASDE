...
from jopm import JointObservationPredictionModel
...
class WorldModelEnv(MultiAgentEnv):
    ...
    def __init__(self,
        ...
        self.initial_joint_observations = self._load_initial_joint_observations(
            initial_joint_observations_path)
    ...
    def _load_initial_joint_observations(self, path):
        # À compléter : chargement des observations initiales (par exemple depuis un fichier)
        # Exemple : np.load(path)
        pass

    def _load_jopm(self, jopm_path) -> 'JointObservationPredictionModel':
        # À compléter : chargement du modèle JOPM sauvegardé (PyTorch)
        # Exemple : torch.load(jopm_path, map_location=device)
        pass
    ...
    def reset(self):
        self.jopm.reset_internal_state(
            batch_size=1, device=torch.device('cpu'))
        idx = random.randint(0, len(self.initial_joint_observations) - 1)
        # On prend directement le dictionnaire d'observation initiale
        self.current_joint_obs = self.initial_joint_observations[idx]
